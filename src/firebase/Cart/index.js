...
function cartLocalStorage(data) {
  localStorage.setItem(
    "cart",
    JSON.stringify({ ...data, localCreatedAt: Date() })
  );
}
  ...
  if (user) {
    ...
    if (localStorage.getItem("cart")) {
      ...
      if (db[0] && localS) {
          ...
          for (let j = 0; j <= localS.items.length - 1; j++) {
            for (let i = 0; i < db[0].data.items.length - 1; i++) {
              if (localS.items[j].id === db[0].data.items[i].id) {
                await editCartFirebase(db[0].uid, {
                  items: arrayRemove(db[0].data.items[i]),
                });
              }
            }
          }
        ...
        data.forEach(
          async (el) =>
            await editCartFirebase(db[0].uid, { items: arrayUnion(el) })
        );
        localStorage.clear();
      } else {
        localS = {
...
export async function editCart(user, item, number) {
  if (user) {
    let cart = await cartOpen(user.uid);
    let allItems = {
      items: cart[0].data.items.map((el) => {
        if (el.id === item.id) {
          return { ...el, quantity: number, updateAt: Date() };
        } else {
          return el;
        }
      }),
...
export async function closeCartFirebase(user) {
  ...
  if (user) {
    let cart = await cartOpen(user.uid);
    let close = { ...cart[0].data, close: true };
